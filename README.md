...
RichTextEdit is a rich text editor, packaged to drop into any project. It is built on
...
- **Any page, no bundler needed**: Blade, jQuery, plain HTML. Load one script
  and one stylesheet, then call `RichTextEdit.mount(...)`.
- **React apps**: import the `<RichTextEdit>` component.
...
- **Safe on Bootstrap pages.** All styles are scoped under `.rte-scope`,
...
```
dist/standalone/rich-text-edit.iife.js
dist/standalone/rich-text-edit.css
```
...
{
  "dependencies": {
    "rich-text-edit": "github:your-org/rich-text-edit#v0.1.0"
  }
}
```
...
2. GitHub Packages only accepts scoped names, so rename the package to
   `@your-org/rich-text-edit` and use that name in the imports below.
3. Add a `publishConfig` pointing at your registry.
4. Run `pnpm run build`, then `pnpm publish`.

After that, projects install it with `pnpm add rich-text-edit` (or the scoped name).
...
```html
<link rel="stylesheet" href="/vendor/rich-text-edit.css">
...
<form method="POST" action="/reports">
  ...
  <!-- The editor mounts here. The rte-scope class is REQUIRED. -->
  <div class="rte-scope" id="report_body_editor"></div>

  <button type="submit">Save</button>
</form>

<script src="/vendor/rich-text-edit.iife.js"></script>
<script>
  RichTextEdit.mount('#report_body_editor', {
...
import "rich-text-edit/style.css";

export function ReportForm({ report }) {
  const editorRef = useRef<RichTextEditHandle>(null);
  ...
  return (
    // The rte-scope class is REQUIRED on a wrapper.
    <div className="rte-scope">
      <RichTextEdit
        ref={editorRef}
...
```js
const value = RichTextEdit.getValue('#box');   // JSON
RichTextEdit.setValue('#box', value);          // same document back
RichTextEdit.getHtml('#box');                  // same document, as HTML
```
...
RichTextEdit.mount('#box', { textarea: '#body' });

// The textarea holds the JSON value as a string.
RichTextEdit.mount('#box', { textarea: '#body', format: 'json' });
```
...
<div class="rte-scope" id="editor"></div>

<script>
  RichTextEdit.mount('#editor', {
    textarea: '#body_json',
    format: 'json',
...
```js
RichTextEdit.setHtml('#editor', storedHtml);
const json = JSON.stringify(RichTextEdit.getValue('#editor'));
```
...
Available as `window.RichTextEdit` (standalone build) or as
`RichTextEditApi` (ESM build). Every function takes the mount container,
...
```ts
import {
  RichTextEdit,        // React component
  RichTextEditApi,     // { mount, destroy, getHtml, setHtml, getValue, setValue }
...
The original web app does this with LibreOffice, in
...
<link rel="stylesheet" href="{{ asset('vendor/rich-text-edit/rich-text-edit.css') }}">
<script src="{{ asset('vendor/rich-text-edit/rich-text-edit.iife.js') }}"></script>
<script>
    RichTextEdit.mount('#report_body_editor', {
        textarea: '#report_body',
...
```js
$('#reportModal').on('shown.bs.modal', () => RichTextEdit.mount('#modal_editor', { textarea: '#modal_body' }));
$('#reportModal').on('hidden.bs.modal', () => RichTextEdit.destroy('#modal_editor'));
```
...
- **Scoped styles.** Every rule is scoped under `.rte-scope`, which is why
...
- **Menus stay styled.** Dropdowns, tooltips and dialogs render into a
  `div.rte-scope.rte-portal` that the editor adds to `<body>`
  itself.
- **Bootstrap-safe.** The build is tuned for **Bootstrap 5** pages: 57 Tailwind
  ...
  output is identical to the original web app's.
...
```js
$('#saveBtn').on('click', () => {
  $.post('/reports/1', { report_body: RichTextEdit.getHtml('#report_body_editor') });
});
```
...
```bash
pnpm install              # dependency versions match the original web app's lockfile
...
| `build:standalone` | `dist/standalone/rich-text-edit.iife.js` (≈2.2 MB, ≈680 KB gzip), `rich-text-edit.css` | Plain HTML / Blade. Everything bundled; sets `window.RichTextEdit`. |
...
| Import | File |
|---|---|
| `rich-text-edit` | `dist/index.js` (+ types) |
| `rich-text-edit/style.css` | `dist/style.css` |
| `rich-text-edit/standalone` | `dist/standalone/rich-text-edit.iife.js` |
| `rich-text-edit/standalone.css` | `dist/standalone/rich-text-edit.css` |
...
│   ├── entry.tsx                 mount API + window.RichTextEdit (standalone entry)
│   ├── RichTextEdit.tsx       the React component, toolbar, HTML/JSON load and save
...
├── build/
│   ├── scope-editor-css.mjs      scopes the CSS under .rte-scope, resolves Bootstrap clashes
...
| Problem | Cause / fix |
|---|---|
| Editor renders unstyled, or menus look broken | The mount container is missing `class="rte-scope"`, or the CSS file isn't loaded. |
| `[RichTextEdit] mount target not found` | `mount` ran before the element existed. Call it after the DOM is ready, or after inserting the markup. |
| `[RichTextEdit] textarea not found` | Wrong `textarea` selector. |
...
- **Flaky browser test.** `tests/browser/font-size-control.mjs`: the two
  ...
  and pass when the file runs alone. The original web app's copy shows the
